...
class _CreateListingScreenState extends State<CreateListingScreen> {
  ...
  XFile? _selectedImage; // Variable to hold the selected image file
  ...
  // --- Submit Logic with Image Upload ---
  Future<void> _submitListing() async {
    ...
    try {
      ...
      await supabase.storage.from('listing_media').upload(
            filePath,
            imageFile,
      ...
      final imageUrl = supabase.storage.from('listing_media').getPublicUrl(filePath);

      // 3. Insert the listing data, including the image URL
      await supabase.from('listings').insert({
        ...
        'type': _selectedType,
        'image_urls': [imageUrl], // Save the URL in the database
      });
